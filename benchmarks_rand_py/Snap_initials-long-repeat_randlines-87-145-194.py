...
def snap(in_str):
	if True:
		out_str = ""
		li = list( re.compile('[a-zA-Z]+(?:\\s+[a-zA-Z]+)*').finditer(in_str) )
		if len(li) < 1:
			return None
		l = list( re.compile('[a-zA-Z]+(?:\\s+[a-zA-Z]+)*').finditer(in_str) )[0].start()
		li = list( re.compile('[a-z]+(?:\\s+[a-z]+)*').finditer(in_str) )
		if len(li) < 1:
			return None
		r = list( re.compile('[a-z]+(?:\\s+[a-z]+)*').finditer(in_str) )[0].start()
		out_str += in_str[l:r]
		out_str += "."
		li = list( re.compile('[A-Z]+(?:\\s+[A-Z]+)*').finditer(in_str) )
		if len(li) < 2:
			return None
		l = list( re.compile('[A-Z]+(?:\\s+[A-Z]+)*').finditer(in_str) )[1].start()
		li = list( re.compile('[a-z]+(?:\\s+[a-z]+)*').finditer(in_str) )
		if len(li) < 1:
			return None
		r = list( re.compile('[a-z]+(?:\\s+[a-z]+)*').finditer(in_str) )[-1].start()
		out_str += in_str[l:r]
		out_str += "."
		return out_str
