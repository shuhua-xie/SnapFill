...
def snap(in_str):
	if True:
		...
		if len(li) < 3:
			return None
		l = list( re.compile('[a-zA-Z0-9]+').finditer(in_str) )[-3].start()
		li = list( re.compile('[a-zA-Z0-9]+').finditer(in_str) )
		if len(li) < 3:
			return None
		r = list( re.compile('[a-zA-Z0-9]+').finditer(in_str) )[-3].end()
		out_str += in_str[l:r]
		out_str += "."
		li = list( re.compile('[a-zA-Z0-9]+').finditer(in_str) )
		if len(li) < 2:
			return None
		l = list( re.compile('[a-zA-Z0-9]+').finditer(in_str) )[-2].start()
		li = list( re.compile('[a-zA-Z0-9]+').finditer(in_str) )
		if len(li) < 2:
			return None
		r = list( re.compile('[a-zA-Z0-9]+').finditer(in_str) )[-2].end()
		out_str += in_str[l:r]
		out_str += "."
		li = list( re.compile('[a-zA-Z0-9]+').finditer(in_str) )
		if len(li) < 1:
			return None
		l = list( re.compile('[a-zA-Z0-9]+').finditer(in_str) )[-1].start()
		li = list( re.compile('[a-zA-Z0-9]+').finditer(in_str) )
		if len(li) < 1:
			return None
		r = list( re.compile('[a-zA-Z0-9]+').finditer(in_str) )[-1].end()
		out_str += in_str[l:r]
		return out_str
