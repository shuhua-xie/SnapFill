...
def snap(in_str):
	if True:
		out_str = ""
		li = list( re.compile( re.escape("+5 528-317-8") ).finditer(in_str) )
		if len(li) < 1:
			return None
		l = list( re.compile( re.escape("+5 528-317-8") ).finditer(in_str) )[-1].end()
		li = list( re.compile( re.escape("+5 528-317-85") ).finditer(in_str) )
		if len(li) < 1:
			return None
		r = list( re.compile( re.escape("+5 528-317-85") ).finditer(in_str) )[-1].end()
		out_str += in_str[l:r]
		return out_str
